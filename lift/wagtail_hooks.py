...
def add_lift_custom(request):
    ...
    if request.method == 'POST':
        try:
            ...
            # Get optional lift_code (manual entry)
            lift_code = data.get('lift_code', '').strip() if data.get('lift_code') else None

            # Validate lift_code uniqueness if provided
            if lift_code and Lift.objects.filter(lift_code=lift_code).exists():
                return JsonResponse({
                    'success': False,
            ...
            lift = Lift.objects.create(
                lift_code=lift_code,  # Can be None
                name=data['name'],
                price=price,
                model=data['model'],
...
def edit_lift_custom(request, lift_code):
    ...
    if request.method == 'POST':
        try:
            ...
            # Update lift_code if provided
            new_lift_code = data.get('lift_code', '').strip() if data.get('lift_code') else None
            if new_lift_code and new_lift_code != lift.lift_code:
