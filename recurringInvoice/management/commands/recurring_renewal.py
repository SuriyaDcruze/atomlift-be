...
class Command(BaseCommand):
    ...
    def list_invoices(self):
        ...
        for invoice in invoices:
            ...
            if days < 0:
                ...
                status = f"[CAUTION] Expires in {days} days"
            else:
    ...
    def renew_invoice(self, reference_id):
        try:
            ...
            if success:
                self.stdout.write(
                    self.style.SUCCESS(
                        f'[SUCCESS] Successfully renewed {reference_id}!'
                    )
                )
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f'[ERROR] Failed to renew {reference_id}'
                    )
                )
        except RecurringInvoice.DoesNotExist:
            self.stdout.write(
                self.style.ERROR(f'[ERROR] Invoice {reference_id} not found')
            )
    ...
    def test_renewal(self):
        ...
        try:
            ...
            if success:
                self.stdout.write(self.style.SUCCESS('[SUCCESS] Renewal test successful!'))
                self.stdout.write(f'  Old End Date: {old_end_date}')
                self.stdout.write(f'  New End Date: {invoice.end_date}')
            else:
                self.stdout.write(self.style.ERROR('[ERROR] Renewal test failed'))
                
        except RecurringInvoice.DoesNotExist:
            self.stdout.write(self.style.ERROR('[ERROR] RINV001 not found'))
